...
#-----------------------------------------------
				...
				if msg.from_ in admin:
					if msg.toType == 2:
						...
						if matched_list == []:
							kk.sendText(msg.to,"Salam sapa manja")
							kc.sendText(msg.to,"kamu memang harus di usir")
							return
				...
				if msg.from_ in admin:
					if msg.toType == 2:
						print "ok"
						_name = msg.text.replace("Hancur","awaw")
						gs = ki.getGroup(msg.to)
						gs = kk.getGroup(msg.to)
						gs = kc.getGroup(msg.to)
						ki.sendText(msg.to,"Aku mau rataik grub g penting ini")
						kk.sendText(msg.to,"Dadah ,marah? Pc own line.me/ti/p/~babydino20.")
						targets = []
						for g in gs.members:
						...
						if targets == []:
							...
							for target in targets:
									try:
										...
										ki.sendText(msg.to,"sukses QQ")
										kk.sendText(msg.to,"Dah Anjing"),
					...
					if msg.toType == 2:
						...
						for g in gs.members:
							if _nametarget == g.displayName:
								targets.append(g.mid)
						if targets == []:
							ki.sendText(msg.to,"gada yang di banned")
							kk.sendText(msg.to,"gada yang di banned")
						else:
							for target in targets:
								try:
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
					ki.sendText(msg.to,"aku disini!!")
#-----------------------------------------------
            elif "Inori say " in msg.text:
					bctxt = msg.text.replace("kamu memang")
...
#-----------------------------------------------
                ...
                if msg.from_ in creator:
                    ...
                    for g in gs.members:
                        if _nametarget == g.displayName:
                            targets.append(g.mid)
                    if targets == []:
                        ki.sendText(msg.to,"Dia g di sini")
                    else:
                        for target in targets:
                            try:
                                admin.append(target)
                                cl.sendText(msg.to,"Selamat Kamu admin")
                            except:
                                pass
                ...
                if msg.from_ in creator:
                    ...
                    for g in gs.members:
                        if _nametarget == g.displayName:
                            targets.append(g.mid)
                    if targets == []:
                        ki.sendText(msg.to,"dia g di sini aku bilang")
                    else:
                        for target in targets:
                            try:
                                admin.remove(target)
            ...
            elif msg.text in ["Cek Admin","adminlist"]:
              if msg.from_ in creator:
                if admin == []:
...
#-----------------------------------------------
              ...
              if msg.from_ in admin:
                 if wait["Protectcancel"] == True:
                     if wait["lang"] == "JP":
                         cl.sendText(msg.to,"Jangan pernah cancel siapapun jika tidak ingin aku marah")
                     else:
                         cl.sendText(msg.to,"done")
                 else:
...
#-----------------------------------------------
            ...
            elif msg.text in ["wkwkwk","wkwk"]:
				if msg.from_ in admin:
					ki.sendText(msg.to,"Sok wkwwk padahal ekspresi datar")
            elif msg.text in ["Bego"]:
				if msg.from_ in admin:
					ki.sendText(msg.to,"Bego􏿿")
					kk.sendText(msg.to,"Bego")
					kc.sendText(msg.to,"Bego")
            elif msg.text in ["#welcome"]:
				if msg.from_ in admin:
					ki.sendText(msg.to,"Hay kamu,selamat bergabung disini")
					kk.sendText(msg.to,"Nakal aku kick")
#-----------------------------------------------
				...
				kc.sendText(msg.to,"􏿿wkwkwkwk")
#-----------------------------------------------
            elif msg.text in ["Responsename","respon"]:
				if msg.from_ in admin:
					ki.sendText(msg.to,"Inori�9�2")
#-----------------------------------------------
...
#------------------------------------------------------------------
				...
				if msg.from_ in admin:
					if wait["blacklist"] == {}:
						cl.sendText(msg.to,"gada")
					else:
						cl.sendText(msg.to,"Di blacklist dia")
						mc = ""
