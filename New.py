...
def bot(op):
    try:
        ...
        if op.type == 17:
                ...
                cb = Message()
                cb.to = op.param1
                cb.text = random.choice(KAC).getContact(op.param2).displayName + "Hay [NewMemb]\n\nSelamat Datang di grub" + random.choice(KAC).getContact(op.param2).displayName + " di [" + group.name + "]\nPatuhi Rules OK!!" + "\n\nCreator => " + group.creator.displayName
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
                    ...
                    kk.sendText(msg.to,"Origami Respon")
#-----------------------------------------------
