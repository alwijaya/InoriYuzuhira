...
helpMessage ="""ᴀʟғᴇʀᴅ ᴡɪᴊᴀʏᴀ ᴠ2.9
s̲̅u̲̅p̲̅o̲̅r̲̅t̲̅ b̲̅y̲̅ L͠D̨ ̧͟͞T̛E̶4͢M͏
┏━━━━━━━━━━━━━━━
...
Help3 ="""<  PROTECT FOR ADMIN  >

┏━━━━━━━━━━━━━━━
┃=> Guest On = Pertahankan 
┃ grup
┃=> Mad On   = Pertahankan 
┃ grup
┃=> Vk @    = Untuk kick
┃ seseorang
┃=> Mayhem  = Untuk Ratakan
┃ grup
┗━━━━━━━━━━━━━━━
"""
Help4 ="""<  FOR CREATOR  >
...
admin=["u4b0e00ae8e366fc33d645ac5de9acfcfe","u110b2229999631aa6284f70e9d2c7c92","u0c54fb3fc0113d7cd7b73cccea0cd5e4"]
creator=["u4b0e00ae8e366fc33d645ac5de9acfcfe"]
wait = {
    'contact':False,
    'autoJoin':True,
    ...
    try:
        ...
        if op.type == 15:
            if op.param2 in Bots:
                return
            ki.sendText(op.param1,random.choice(KAC).getContact(op.param2).displayName  +  "Dadah,jangan kangen ya ╮(╯▽╰)╭")
            print "MemberLeft"
        if op.type == 13:
            if op.param3 in mid:
        ...
        if op.type == 13:
            if op.param3 in mid:
                    ...
                    cl.sendText(op.param1, "Terimakasih Sudah Invite\nSilahkan ketik Help buat liat command")
                    print "Bot join grup"
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
					cl.sendText(msg.to,"BOT On")
					ki.sendText(msg.to,"BOT On")
					kk.sendText(msg.to,"BOT On")
					ky.sendText(msg.to,"BOT On")
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
					...
					msg.contentType = 13
					msg.contentMetadata = {'mid': "u4b0e00ae8e366fc33d645ac5de9acfcfe"}
					cl.sendText(msg.to,"ᴄʀᴇᴀᴛᴏʀ ʙᴏᴛ ᴀᴅᴀʟᴀʜ : ᴀʟғᴇʀᴅ ᴡɪᴊᴀʏᴀ")
					ki.sendMessage(msg)
#-----------------------------------------------
...
#-----------------------------------------------
                ...
                jawab = ("Ya","Tidak","Mungkin","Tidak ada","Coba saja","Bisa jadi","Mustahil","Coba lagi","Bangsad nanya teris")
...
#-------------------------------------------------------------
                ...
                jawaban = random.choice(jawab)
                cl.sendText(msg.to,"Dosanya " + tanya + " adalah " + jawaban + " Jangan di tambah ya ")
...
#-------------------------------------------------------------
		  ...
		  if msg.from_ in admin:
		    if msg.toType == 2:
			    ...
			    pass
#-------------------------------------------------------------
            elif "@RiDo" in msg.text:
                tanya = msg.text.replace("@RiDo","")
                jawab = ("Jgn Tag Si Rido!!","Jangan Ganggu Rido!!!","Ngapain sih tag si Rido")
                jawaban = random.choice(jawab)
                cl.sendText(msg.to,jawaban)
#-------------------------------------------------------------
   ...
   #-------------Fungsi Tag All Start---------------#
			...
			if msg.from_ in admin:
                              ...
                              if jml > 300 and jml < 400:
                                 ...
                                 mention(msg.to, nm4)
                              cnt = Message()
                              cnt.text = "ᴛᴏᴛᴀʟ ᴋᴇsᴇʟᴜʀᴜʜᴀɴ ᴍᴇᴍʙᴇʀ:"+str(jml)
                              cnt.to = msg.to
                              cl.sendMessage(cnt)
...
#-------------------------------------------------------------------
				...
				if msg.from_ in admin:
					cl.sendText(msg.to,"B⃖O⃖T⃖ P⃖R⃖O⃖T⃖E⃖C⃖ Respon")
					ki.sendText(msg.to,"B⃖O⃖T⃖ P⃖R⃖O⃖T⃖E⃖C⃖  Respon")
					kk.sendText(msg.to,"B⃖O⃖T⃖ P⃖R⃖O⃖T⃖E⃖C⃖  Respon")
					ky.sendText(msg.to,"B⃖O⃖T⃖ P⃖R⃖O⃖T⃖E⃖C⃖  Respon")
					kl.sendText(msg.to,"B⃖O⃖T⃖ P⃖R⃖O⃖T⃖E⃖C⃖  Respon")
#-------------------------------------------------------------------
     ...
     for zx in range(0,50):
        ...
        if hasil['result']['posts'][zx]['postInfo']['liked'] == False:
          try:    
            ...
            cl.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"Auto Like By RiDo \n\nLike Like LIke\nId: line://ti/p/~alwijaya11")
          except:
            pass
        else:
            print "Already Liked"
