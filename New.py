...
creator=["uddb6b470ab4b54c5fbc69d7600cc3bc81a"]
wait = {
    'contact':False,
    'autoJoin':True,
...
def bot(op):
    try:
        ...
        if op.type == 17:
            ...
            ki.sendText(op.param1, "selamat datang di grub " + "[" + str(ginfo.name)) + "]"
            ki.sendText(op.param1, "#OwnerGrup# " + str(ginfo.name) + " :\n" + ginfo.creator.displayName)
            print "MemberJoin"
        if op.type == 15:
            if op.param2 in Bots:
                return
            ki.sendText(op.param1, "Dadah semoga tenang disana (*._.*)")
            print "MemberLeft"
        ...
        if op.type == 26:
            ...
            if msg.contentType == 13:
               if wait["wblack"] == True:
				...
				if msg.from_ in admin:
					if msg.toType == 2:
						X = cl.getGroup(msg.to)
						X.name = msg.text.replace("Origami gn ","")
						ki.updateGroup(X)
					else:
             ...
             = {'mid': Bmid}
				...
				if msg.from_ in admin:
					...
					msg.text = None
					cl.sendMessage(msg)
            elif msg.text in ["Ã¦â€žâ„1¤7ºÃ£ÂÂ®Ã£Æ’â„1¤7”Ã£Æ’Â¬Ã£â„1¤7šÂ¼Ã£Æ’Â³Ã£Æ’Ë„1¤7","Origami gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
					...
					msg.text = None
					ki.sendMessage(msg)
            elif msg.text in ["Ã¦â€žâ„1¤7ºÃ£ÂÂ®Ã£Æ’â„1¤7”Ã£Æ’Â¬Ã£â„1¤7šÂ¼Ã£Æ’Â³Ã£Æ’Ë„1¤7","Tohka gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
					...
					msg.text = None
					kk.sendMessage(msg)
            elif msg.text in ["Ã¦â€žâ„1¤7ºÃ£ÂÂ®Ã£Æ’â„1¤7”Ã£Æ’Â¬Ã£â„1¤7šÂ¼Ã£Æ’Â³Ã£Æ’Ë„1¤7","Kurumi gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
				...
				if msg.from_ in admin:
					if msg.toType == 2:
						...
						kk.updateGroup(X)
						if wait["lang"] == "JP":
							kk.sendText(msg.to,"Done Origami")
						else:
				...
				if msg.from_ in admin:
					if msg.toType == 2:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"Not for use less than group")
            elif msg.text in ["Miku curl","Miku link off"]:
				if msg.from_ in admin:
					if msg.toType == 2:
            ...
            elif msg.text in ["Origami curl","Origami link off"]:
				if msg.from_ in admin:
					if msg.toType == 2:
						X = kk.getGroup(msg.to)
						X.preventJoinByTicket = True
						...
						if wait["lang"] == "JP":
							...
							kk.sendText(msg.to,"Not for use less than group")
            elif msg.text in ["Tohka curl","Tohka link off"]:
				if msg.from_ in admin:
					if msg.toType == 2:
						X = kc.getGroup(msg.to)
						X.preventJoinByTicket = True
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
					if msg.toType == 2:
						...
						if matched_list == []:
							kk.sendText(msg.to,"Bye Anjing")
							kc.sendText(msg.to,"Blacklist emang pantes di usir")
							return
				...
				if msg.from_ in admin:
					if msg.toType == 2:
						...
						ki.sendText(msg.to,"Perintah penghancuran grub di laksanakan")
						kk.sendText(msg.to,"Grub akan di ratakan dalam beberapa detik......")
                        kk.sendText(msg.to,"Open sewa bot rental untuk jaga grub dari kicker , minat pc owner line.me/ti/p/~babydino20")
						targets = []
						for g in gs.members:
						...
						if targets == []:
							...
							for target in targets:
								try:
									...
									ki.sendText(msg.to,"Grub hancur")
            elif "Nk " in msg.text:
				if msg.from_ in admin:
					if msg.from_ in admin:
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
                    ...
                    kc.sendText(msg.to,"Hadir Boss!!")
#-----------------------------------------------
            elif "Miku say " in msg.text:
					bctxt = msg.text.replace("Miku say ","")
...
#-----------------------------------------------
              ...
              if msg.from_ in admin:
                 if wait["Protectcancel"] == True:
                     if wait["lang"] == "JP":
                         cl.sendText(msg.to,"jANGAN CANCEL SIAPAPUN JIKA TIDAK INGIN AKU MARAH!")
                     else:
                         cl.sendText(msg.to,"done")
                 else:
...
#-----------------------------------------------
				...
				kk.sendText(msg.to,"Tapi sayang hatimu busuk")
				kc.sendText(msg.to,"wkwkwkw")
#-----------------------------------------------
			...
			for zx in range(0,20):
				...
				if hasil['result']['posts'][zx]['postInfo']['liked'] == False:
					try:    
						...
						kk.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"autolike by @pancy_bot")
						print "DiLike"
					except:
							pass
				else:
