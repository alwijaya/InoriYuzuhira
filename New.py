...
cl = TOBY.LINE()
cl.login(token="EmiglVSQV8Kfq1aK2Sr6.GyD7/7Pd64TIBouM4OHYTG.p9Coe80tkMHCaC0zsvUInfilHB7cnGaOV1CoOUGx0E8=")
cl.loginResult

ki = TOBY.LINE()
ki.login(token="Em5eQDOnUNMn6g1SK59a.2VkhNbXAiKA+FT3J3JVdUG.vlHVkOCk4zAnlwsojeTbH1/za0jEs+d4p5pc64XiKFo=")
ki.loginResult
...
♱ ʟɪsᴛᴛᴀʀɢᴇᴛ
        ...
        C⃟O⃟M⃟M⃟A⃟N⃟D⃟ P⃟R⃟O⃟T⃟E⃟C⃟T⃟ G⃟R⃟O⃟U⃟P⃟
☠ ɢᴜᴇsᴛ ᴏɴ/ᴏғғ
☠ ᴍᴀᴅ ᴏɴ/ᴏғғ
☠ ǫʀ ᴏɴ/ᴏғғ
☠ ᴘʀᴏᴛᴇᴄᴛ ᴏɴ/ᴏғғ
...
def bot(op):
    try:
        ...
        if op.type == 17:
                ...
                cb = Message()
                cb.to = op.param1
                cb.text = random.choice(KAC).getContact(op.param2).displayName + "Hay \n\nSelamat  Datang" + random.choice(KAC).getContact(op.param2).displayName + " di [" + group.name + "]\nPatuhi Rules OK!!" + "\n\nPembuat grup => " + group.creator.displayName
                random.choice(KAC).sendMessage(cb)
        if op.type == 15:
            if op.param2 in Bots:
                return
            ki.sendText(op.param1, "Selamat jalan , semoga tenang qq")
            print "MemberLeft"
        if op.type == 13:
            if op.param3 in mid:
        ...
        if op.type == 26:
            ...
            if msg.contentType == 13:
               if wait["wblack"] == True:
            ...
            elif "Miku kick " in msg.text:
				if msg.from_ in admin:
					midd = msg.text.replace("Miku kick ","")
					ki.kickoutFromGroup(msg.to,[midd])
            elif "Tohka kick " in msg.text:
				if msg.from_ in admin:
					midd = msg.text.replace("Tohka kick ","")
					kk.kickoutFromGroup(msg.to,[midd])
            elif "Origami kick " in msg.text:
				if msg.from_ in admin:
					midd = msg.text.replace("Origami kick ","")
            ...
            elif "Miku invite " in msg.text:
				if msg.from_ in admin:
					midd = msg.text.replace("Miku invite ","")
					ki.findAndAddContactsByMid(midd)
					ki.inviteIntoGroup(msg.to,[midd])
            elif "Tohka invite " in msg.text:
				if msg.from_ in admin:
					midd = msg.text.replace("Tohka invite ","")
					kk.findAndAddContactsByMid(midd)
					kk.inviteIntoGroup(msg.to,[midd])
            elif "Origami invite " in msg.text:
				if msg.from_ in admin:
					midd = msg.text.replace("Origami invite ","")
				...
				if msg.from_ in admin:
					...
					msg.text = None
					cl.sendMessage(msg)
            elif msg.text in ["æ„�1�7�ã®ãƒ�1�7�ãƒ¬ã�1�7�¼ãƒ³ãƒ˄1�7","Miku gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
					...
					msg.text = None
					ki.sendMessage(msg)
            elif msg.text in ["æ„�1�7�ã®ãƒ�1�7�ãƒ¬ã�1�7�¼ãƒ³ãƒ˄1�7","Tohka gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
					...
					msg.text = None
					kk.sendMessage(msg)
            elif msg.text in ["æ„�1�7�ã®ãƒ�1�7�ãƒ¬ã�1�7�¼ãƒ³ãƒ˄1�7","Origami gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
				...
				if msg.from_ in admin:
					if msg.toType == 2:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"Not for use less than group")
            elif msg.text in ["Miku ourl","Miku link on"]:
				if msg.from_ in admin:
					if msg.toType == 2:
						X = cl.getGroup(msg.to)
						X.preventJoinByTicket = False
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"Not for use less than group")
            elif msg.text in ["Tohka ourl","Tohka link on"]:
				if msg.from_ in admin:
					if msg.toType == 2:
            ...
            elif msg.text in ["Origami ourl","Origami link on"]:
				if msg.from_ in admin:
					if msg.toType == 2:
						...
						if targets == []:
							...
							for target in targets:
								try:
									...
									ki.sendText(msg.to,"Error")
            elif "Hsampah @" in msg.text:
				if msg.from_ in admin:
					if msg.toType == 2:
						print "[Unban]ok"
						_name = msg.text.replace("Hsampah @","")
						_nametarget = _name.rstrip('  ')
						gs = cl.getGroup(msg.to)
						gs = ki.getGroup(msg.to)
              ...
              if msg.from_ in admin:
                ...
                for target in targets:
                   try:
                      ...
                      pass
#-----------------------------------------------
            elif msg.text in ["Siap"]:
				if msg.from_ in admin:
					cl.sendText(msg.to,"Miku siap")
					ki.sendText(msg.to,"Tohka siap")
                    kk.sendText(msg.to,"Origami siap")
...
#-----------------------------------------------
					...
					msg.contentType = 13
					msg.contentMetadata = {'mid': "uddb6b470ab4b54c5fbc69d7600cc3bc811"}
...
#-----------------------------------------------
                ...
                if msg.from_ in creator:
                    ...
                    cl.sendText(msg.to,"Command DiTolak")
                    ki.sendText(msg.to,"Kamu g bisa make")
                    cl.sendText(msg.to,"Admin gaush sok")
                    ki.sendText(msg.to,"wkwk")

            elif msg.text in ["Adminlist","adminlist"]:
              if msg.from_ in creator:
                if admin == []:
                    cl.sendText(msg.to,"Adminnya belum ada")
                    ki.sendText(msg.to,"tambahin dong")
                    ki.sendText(msg.to,"wkwk")
                else:
...
#-----------------------------------------------
              ...
              if msg.from_ in admin:
                 if wait["Protectcancel"] == True:
                     if wait["lang"] == "JP":
                         cl.sendText(msg.to,"Aku peringatkan")
                         ki.sendText(msg.to,"Jgn cancel undangan atau autokick!")
                         kk.sendText(msg.to,"karna aku akan marah")
                     else:
                         cl.sendText(msg.to,"done")
...
#-----------------------------------------------
            ...
            elif msg.text in ["Alferd"]:
				if msg.from_ in admin:
					ki.sendText(msg.to,"Baik")
					kk.sendText(msg.to,"Imut")
					kc.sendText(msg.to,"Ganteng")
            elif msg.text in ["#welcome"]:
				if msg.from_ in admin:
					ki.sendText(msg.to,"Hay kamu, cek note ya")
					kk.sendText(msg.to,"Patuhi rules,langgar?AUTOKICK")
#-----------------------------------------------
            elif msg.text in ["PING","Ping","ping","Samlekom","samlekom"]:
				ki.sendText(msg.to,"Muka Kamu imut􏿿")
				kk.sendText(msg.to,"Tapi sayang hatimu BUSUK􏿿")
				kc.sendText(msg.to,"kaya mantan ku􏿿")
#-----------------------------------------------
            elif msg.text in ["Responsename","Respon"]:
				if msg.from_ in admin:
					cl.sendText(msg.to,"Miku Respon")
					ki.sendText(msg.to,"Tohka Respon")
#-----------------------------------------------
            ...
            for zx in range(0,20):
                ...
                if hasil['result']['posts'][zx]['postInfo']['liked'] == False:
                    try:    
                        ...
                        kk.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"Tercyduck oleh @pancy_bot")
                        print "DiLike"
                    except:
                            pass
                else:
