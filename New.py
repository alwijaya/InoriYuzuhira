...
admin=["uddb6b470ab4b54c5fbc69d7600cc3bc8","u0c54fb3fc0113d7cd7b73cccea0cd5e4"]
...
wait = {
    ...
    'timeline':True,
    'autoAdd':True,
    'message':"Owner : line://ti/p/~babydino11",
    "lang":"JP",
    "comment":"Owner : line://ti/p/~babydino11",
    "commentOn":True,
    "commentBlack":{},
    "wblack":False,
    "dblack":False,
...
def bot(op):
    try:
        ...
        if op.type == 17:
                ...
                cb = Message()
                cb.to = op.param1
                cb.text = random.choice(KAC).getContact(op.param2).displayName + " Hay \n\nSelamat Datang ya  ♥(✿ฺ´∀`✿ฺ)ﾉ" + random.choice(KAC).getContact(op.param2).displayName + " di [" + group.name + "]\nPatuhi rules ya qq " + "\n\nPembuat grup => " + group.creator.displayName
                random.choice(KAC).sendMessage(cb)
        if op.type == 15:
            if op.param2 in Bots:
                return
            ki.sendText(op.param1, "Sedih kamu pergi. tapi bagus deh jdi g nyesek nyesekin grup ヾ(´･ ･｀｡)ノ")
            print "MemberLeft"
        if op.type == 13:
            if op.param3 in mid:
              ...
              if msg.from_ in admin:
				...
				if msg.from_ in admin:
                    ...
                    if msg.to in wait2['readPoint']:
                        ...
                        cl.sendText(msg.to, "Daftar orang sider %s\nthat's it\n\nDaftar orang diser g nongol di grub\n%sMalu maluin,Minta join di grub nangis nangis taunya cumn jadi kaum sider 7¬8\n\nReading point creation date n time:\n[%s]"  % (wait2['readMember'][msg.to],chiya,setTime[msg.to]))
                    else:
...
#-----------------------------------------------
              ...
              if msg.from_ in admin:
                if msg.toType == 2:
                    ...
                    cl.sendText(msg.to,"¡¸ Hay anjing ¡¹\nProses akan di mulai\n' abort' to abort7¬8")
                    ki.sendText(msg.to,"¡¸ Bangsad ¡¹\nyololololoololfrjhgrbkrvrtbg ofjndj7¬8\n/0·5h0¬5l0¬5b0¬50·4lo0Æ0o,0·4h0¬5l0¬5b0¬50·5lo0Æ0o/")
                    targets = []
                    for g in gs.members:
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
					cl.sendText(msg.to,"Tohka")
					ki.sendText(msg.to,"Miku")
					kb.sendText(msg.to,"Kurumi")
					ky.sendText(msg.to,"Origami")
					cl.sendText(msg.to,"Kami semua siap play jika di suruh wkwk")
#-----------------------------------------------
				...
				if msg.from_ in admin:
					...
					cl.sendText(msg.to,"MyCreator: Pancy")
					ki.sendMessage(msg)
					msg.contentType = 13
					msg.contentMetadata = {'mid': "uddb6b470ab4b54c5fbc69d7600cc3bc8"}
					kb.sendText(msg.to,"MyCreator: Pancy")
					ky.sendMessage(msg)
...
#-----------------------------------------------
              ...
              if msg.from_ in admin:
                 if wait["Protectcancel"] == True:
                     if wait["lang"] == "JP":
                         cl.sendText(msg.to,"Kumohon kalian baik baik di grub dan jangan buat aku marah")
...
#-----------------------------------------------
				...
				kb.sendText(msg.to,"Tapi sayangnya")
				ky.sendText(msg.to,"Hatimu busuk")
#-----------------------------------------------
            elif msg.text in ["Responsename","Absen bot"]:
				if msg.from_ in admin:
					cl.sendText(msg.to,"Tohka Yatogami")
					ki.sendText(msg.to,"Miku Izayoi")
					kb.sendText(msg.to,"Kurumi Tokisaki")
					ky.sendText(msg.to,"Origami Tobiichi")
#-----------------------------------------------
			...
			for zx in range(0,20):
				...
				if hasil['result']['posts'][zx]['postInfo']['liked'] == False:
					try:    
						...
						kk.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"Trcyduck oleh @pancy_bot")
						print "DiLike"
					except:
							pass
				else:
