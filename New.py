...
def bot(op):
    try:
        ...
        if op.type == 17:
                ...
                cb = Message()
                cb.to = op.param1
                cb.text = random.choice(KAC).getContact(op.param2).displayName + " \n\n Selamat Datang  ♥(✿ฺ´∀`✿ฺ)ﾉ " + random.choice(KAC).getContact(op.param2).displayName + " di [" + group.name + "]\nPatuhi rules + cek Note!!" + "\n\nPembuat grub ini adalah => " + group.creator.displayName
                random.choice(KAC).sendMessage(cb)
        if op.type == 15:
            if op.param2 in Bots:
                return
            ki.sendText(op.param1, "Semoga tenang disana ヾ(´･ ･｀｡)ノ")
            print "MemberLeft"
        if op.type == 13:
            if op.param3 in mid:
              ...
              if msg.from_ in admin:
				...
				if msg.from_ in admin:
                    ...
                    if msg.to in wait2['readPoint']:
                        ...
                        cl.sendText(msg.to, "Yang ngeread dan muncul %s\nthat's it\n\nYang cuman nyider\n%sLu keluar sana, buat apa punya grub tapi kerjanya nyider doang 7¬8\n\nReading point creation date n time:\n[%s]"  % (wait2['readMember'][msg.to],chiya,setTime[msg.to]))
                    else:
...
#.acceptGroupInvitationByTicket(msg.to,Ticket)
				...
				if msg.from_ in admin:
							...
							kc.updateGroup(G)
#-----------------------------------------------
            elif msg.text in ["Out","dadah semua"]:
				if msg.from_ in admin:
					if msg.toType == 2:
						ginfo = cl.getGroup(msg.to)
						try:
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
					cl.sendText(msg.to,"Tohka hadir Boss")
					ki.sendText(msg.to,"Miku juga hadir Boss")
                    kk.sendText(msg.to,"Kurumi juga hadir Boss")
                    kc.sendText(msg.to,"Origami juga hadir Boss")
#-----------------------------------------------
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
					ki.sendText(msg.to,"Cogan cina")
					kk.sendText(msg.to,"orangnya baik")
					kc.sendText(msg.to,"Dan segalanya")
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
					...
					kc.sendText(msg.to,"Origami Tobiichi")
#-----------------------------------------------
